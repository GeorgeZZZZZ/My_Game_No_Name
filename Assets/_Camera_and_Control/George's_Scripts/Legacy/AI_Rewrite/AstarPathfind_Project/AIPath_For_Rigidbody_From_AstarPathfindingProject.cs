...
public class AIPath_For_Rigidbody_From_AstarPathfindingProject : MonoBehaviour {
	...
	public float TrySearchPath () {
		...
		if (Time.time - lastRepath >= repathRate && canSearchAgain && canSearch && newTar != null && newTar != oldTar && canMove) {
			oldTar = newTar;
			SearchPath();
			return repathRate;
		} else {
	...
	public virtual void SearchPath () {
		...
		if (newTar == null) throw new System.InvalidOperationException("Target is null");

		lastRepath = Time.time;

		canSearchAgain = false;

		seeker.StartPath(GetFeetPosition(), newTar);
	}
